...
export default function Connectors() {
  ...
  const connectGmail = async () => {
    ...
    await signIn("google", { callbackUrl: `${window.location.origin}/#connectors` });
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <Reveal className="mb-10 max-w-3xl">
          ...
          <p className="mt-3 text-sm leading-relaxed text-[var(--shield-text-dim)] sm:text-base">
            AbroadShield shows the real integration state. Connected means authenticated and usable; External means the platform can be opened but AbroadShield does not claim direct access.
          </p>
        </Reveal>
          ...
          <div className="flex flex-wrap gap-1.5">
            {CATEGORIES.map((category) => (
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-xs font-medium capitalize transition ${filter === category ? "border-[oklch(0.74_0.17_162/0.5)] bg-[oklch(0.74_0.17_162/0.12)] text-[oklch(0.85_0.19_158)]" : "border-[var(--shield-border)] bg-[oklch(0.22_0.025_165/0.4)] text-[var(--shield-text-dim)] hover:text-[var(--shield-text)]"}`}
              >
                {category}
              </button>
            ))}
            ...
            {visible.map((connector, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] p-5"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--shield-border)] bg-[oklch(0.74_0.17_162/0.08)]">
                      {connector.category === "jobs" ? <Briefcase className="h-4 w-4" /> : connector.category === "email" ? <Mail className="h-4 w-4" /> : connector.category === "housing" ? <Home className="h-4 w-4" /> : connector.category === "education" ? <GraduationCap className="h-4 w-4" /> : connector.category === "travel" ? <Plane className="h-4 w-4" /> : connector.category === "finance" ? <CreditCard className="h-4 w-4" /> : <Smartphone className="h-4 w-4" />}
                    </div>
                    <span className={`rounded-full border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${connected ? "border-[oklch(0.74_0.17_162/0.4)] bg-[oklch(0.74_0.17_162/0.1)] text-[oklch(0.85_0.19_158)]" : "border-[var(--shield-border)] text-[var(--shield-text-faint)]"}`}>
                      {connected ? "Connected" : connector.status === "oauth" ? "Not connected" : "External"}
                    </span>
                  </div>
                  <h3 className="mt-4 text-sm font-semibold text-[var(--shield-text)]">{connector.name}</h3>
                  <p className="mt-1.5 min-h-12 text-xs leading-relaxed text-[var(--shield-text-dim)]">{connector.description}</p>

                  {connector.id === "gmail" ? (
                    <button
                      onClick={connectGmail}
                      disabled={loadingGmail || connected}
                      className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-[oklch(0.74_0.17_162/0.35)] bg-[oklch(0.74_0.17_162/0.08)] px-3 py-2 text-xs font-semibold text-[oklch(0.85_0.19_158)] disabled:opacity-50"
                    >
                      {connected ? <CheckCircle2 className="h-3.5 w-3.5" /> : <ShieldCheck className="h-3.5 w-3.5" />}
                      {connected ? "Gmail connected" : loadingGmail ? "Connecting…" : "Connect Gmail"}
                    </button>
                  ) : (
                    <a
                      ...
                      className={`mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-[var(--shield-border)] px-3 py-2 text-xs font-semibold text-[var(--shield-text-dim)] ${connector.url === "#" ? "cursor-not-allowed opacity-50" : "hover:text-[var(--shield-text)]"}`}
                    >
        ...
        <div className="mt-8 rounded-2xl border border-dashed border-[var(--shield-border)] p-4 text-center text-xs text-[var(--shield-text-dim)]">
          <ShieldCheck className="mr-1.5 inline h-3.5 w-3.5" />
          Approval remains required before outbound actions. No connector is presented as active unless the application can verify it.
        </div>
      </div>
    </section>
  );
