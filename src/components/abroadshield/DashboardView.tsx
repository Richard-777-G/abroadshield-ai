...
export default function DashboardView({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  useEffect(() => {
    ...
    ]).then(([items, next]) => { if (!cancelled) { setRequirements(items); setNextAction(next); } }).catch(() => {});
  ...
  return <div className="mx-auto w-full max-w-6xl px-5 py-6 sm:px-8 sm:py-7">
    <div className="rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] p-5 sm:p-6">
      ...
      <div className="mt-6 rounded-xl border border-[var(--shield-border)] bg-[oklch(0.14_0.018_165)] p-4"><div className="flex items-center justify-between gap-4"><div><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--shield-text-faint)]">Journey map</div><div className="mt-1 text-sm font-semibold">Your stages are context, not navigation.</div><div className="mt-1 text-xs text-[var(--shield-text-dim)]">The stage cards below are informational. They never change screens. Use <span className="font-medium text-[var(--shield-text)]">Open Journey</span> when you explicitly want the Journey workspace.</div></div><div className="hidden text-[9px] uppercase tracking-[0.14em] text-[var(--shield-text-faint)] sm:block">Current: {policy.title}</div></div><div className="mt-4 grid gap-2 sm:grid-cols-4">{PHASES.map((p,index)=>{const current=index===phaseIndex;return <div key={p.id} className={"rounded-xl border p-3 "+(current?"border-[oklch(0.74_0.17_162/0.45)] bg-[oklch(0.74_0.17_162/0.08)]":"border-[var(--shield-border)]")}><div className="flex items-center justify-between"><span className="text-[10px] font-semibold">{String(index+1).padStart(2,"0")}</span><span className="text-[9px] uppercase text-[var(--shield-text-faint)]">{current?"Current":index>phaseIndex?"Next":"Earlier"}</span></div><div className="mt-2 text-xs font-semibold">{p.name}</div><div className="mt-1 line-clamp-2 text-[10px] leading-4 text-[var(--shield-text-faint)]">{getStagePolicy(p.id).mission}</div></div>})}</div></div>
