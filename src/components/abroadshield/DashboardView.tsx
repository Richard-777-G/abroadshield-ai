...
export default function DashboardView({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  async function run(type: keyof typeof ACTION_META) {
    ...
    try {
      ...
      const r = await fetch("/api/abroadshield/tasks", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ taskType: type, context: action.context }),
      });
      const d = await r.json();
      setResult({
        label: action.label,
        text: d.ok ? (typeof d.result === "string" ? d.result : JSON.stringify(d.result, null, 2)) : (d.error || "Task failed"),
        ok: Boolean(d.ok),
      });
      if (d.ok) {
      ...
      <div className="rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] p-5 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-1 text-2xl font-semibold">What matters now.</h1>
            <p className="mt-1 text-xs text-[var(--shield-text-dim)]">{profile.destination || "Your destination"} · {policy.title} · {ready}% document readiness</p>
          </div>
          <button type="button" onClick={() => onNavigate("agent")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[oklch(0.74_0.17_162)] px-4 py-2.5 text-xs font-semibold text-[oklch(0.12_0.016_165)]">
            ...
            <div className="mt-1 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              {nextAction.type && nextAction.type in ACTION_META ? (
                <button type="button" disabled={!!working} onClick={() => void run(nextAction.type as keyof typeof ACTION_META)} className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-[oklch(0.74_0.17_162)] px-4 py-2.5 text-xs font-semibold text-[oklch(0.12_0.016_165)]">
                  {working === nextAction.type ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ArrowRight className="h-3.5 w-3.5" />}Take action
                </button>
              ) : null}
            </div>
          </div>
        ) : (
          <div className="mt-6 rounded-xl border border-[var(--shield-border)] p-4 text-xs text-[var(--shield-text-dim)]">No active action yet. Use the Journey workspace deliberately when you are ready.</div>
        )}

        <div className="mt-6 rounded-xl border border-[var(--shield-border)] p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {actions.map((type) => (
              <button key={type} type="button" disabled={!!working} onClick={() => void run(type)} className="rounded-full border border-[var(--shield-border)] px-3 py-1.5 text-[10px] text-[var(--shield-text-dim)] hover:border-[oklch(0.74_0.17_162/0.35)] disabled:opacity-60">{ACTION_META[type].label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-[var(--shield-border)] bg-[oklch(0.14_0.018_165)] p-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--shield-text-faint)]">Journey map</div>
              <div className="mt-1 text-sm font-semibold">Your stages are context, not navigation.</div>
              <div className="mt-1 text-xs text-[var(--shield-text-dim)]">The stage cards below are informational. They never change screens. Use <span className="font-medium text-[var(--shield-text)]">Open Journey</span> when you explicitly want the Journey workspace.</div>
            </div>
            <div className="hidden text-[9px] uppercase tracking-[0.14em] text-[var(--shield-text-faint)] sm:block">Current: {policy.title}</div>
          </div>
          <div className="mt-4 grid gap-2 sm:grid-cols-4">
            {PHASES.map((p, index) => {
              const current = index === phaseIndex;
              return (
                <div key={p.id} className={`rounded-xl border p-3 ${current ? "border-[oklch(0.74_0.17_162/0.45)] bg-[oklch(0.74_0.17_162/0.08)]" : "border-[var(--shield-border)]"}`}>
                  <div className="flex items-center justify-between"><span className="text-[10px] font-semibold">{String(index + 1).padStart(2, "0")}</span><span className="text-[9px] uppercase text-[var(--shield-text-faint)]">{current ? "Current" : index > phaseIndex ? "Next" : "Earlier"}</span></div>
                  <div className="mt-2 text-xs font-semibold">{p.name}</div>
                  <div className="mt-1 line-clamp-2 text-[10px] leading-4 text-[var(--shield-text-faint)]">{getStagePolicy(p.id).mission}</div>
                </div>
              );
            })}
          </div>
        ...
        {requirements.length > 0 && (
          <div className="mt-5 rounded-xl border border-[var(--shield-border)] bg-[oklch(0.14_0.018_165)] p-4">
            <div className="flex items-center justify-between"><div className="text-xs font-semibold">Needs attention now</div><button type="button" onClick={() => onNavigate("journey")} className="text-[10px] text-[oklch(0.74_0.17_162)]">Open journey →</button></div>
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {requirements.map((item) => <div key={item.title} className="flex items-center gap-2 rounded-lg border border-[var(--shield-border)] px-3 py-2"><AlertTriangle className="h-3.5 w-3.5 shrink-0 text-[oklch(0.8_0.15_80)]" /><span className="min-w-0 flex-1 truncate text-[11px]">{item.title}</span><span className="text-[9px] uppercase text-[var(--shield-text-faint)]">{item.priority}</span></div>)}
            </div>
          </div>
        )}

        {result && (
          <div className={`mt-5 rounded-xl border p-4 ${result.ok ? "border-[oklch(0.74_0.17_162/0.3)] bg-[oklch(0.74_0.17_162/0.05)]" : "border-red-500/20 bg-red-500/5"}`}>
            ...
            <pre className="max-h-80 overflow-auto whitespace-pre-wrap text-[11px] leading-5 text-[var(--shield-text-dim)]">{result.text}</pre>
          </div>
        )}
      </div>
    </div>
