...
import PolicyEvidenceCard, { type PolicyEvidenceView } from "./PolicyEvidenceCard";
import type { PhaseId } from "@/lib/abroadshield/phase";

type DashboardSnapshot = {
  profile: { goal: string; destination: string | null; course: string | null; university: string | null; intake: string | null };
  ...
  readiness: number;
  next?: { id?: string; title: string; reason?: string; type?: string; status?: string; priority?: number; dueAt?: string | Date | null; result?: unknown; capability?: string } | null;
  activeCount: number;
  blockedCount: number;
...
export default function DashboardView({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  if (!snapshot) {
    return <section className="w-full"><div className="mx-auto flex min-h-[420px] w-full max-w-7xl items-center justify-center px-5 py-7 sm:px-8 lg:px-10 lg:py-9"><div className="flex items-center gap-2 text-xs text-[var(--shield-text-dim)]">{loading && <Loader2 className="h-4 w-4 animate-spin"/>}{loading ? "Reading your journey…" : "Your journey data is not available yet."}</div></div></section>;
  }
