    ...
    <section id="top" className="relative min-h-[100svh] w-full overflow-hidden bg-transparent">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-7xl flex-col px-6 sm:px-10">
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7, delay: .1 }} className="mt-24 flex items-center justify-between sm:mt-28">
          ...
          <LanguageToggle locale={locale} onChange={() => undefined} />
        </motion.div>
        ...
        <div className="mt-8 grid flex-1 items-center gap-12 pb-10 lg:grid-cols-[.92fr_1.08fr] lg:gap-14 lg:pb-14">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8, delay: .15 }}>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <button type="button" onClick={() => onNavigate?.("journey")} className="inline-flex items-center gap-2 rounded-full bg-[oklch(0.98_0.005_160)] px-5 py-3 text-sm font-semibold text-[oklch(0.14_0.018_165)] transition hover:-translate-y-0.5 hover:bg-white"><BrainCircuit className="h-4 w-4" />See how the agent works<ArrowRight className="h-3.5 w-3.5" /></button>
              <button type="button" onClick={() => onNavigate?.("journey")} className="inline-flex items-center gap-2 rounded-full border border-[oklch(0.6_0.04_165/0.22)] px-5 py-3 text-sm font-semibold text-[oklch(0.88_0.005_180)] transition hover:-translate-y-0.5 hover:bg-[oklch(0.24_0.028_165/0.5)]"><Target className="h-4 w-4" />Explore the blueprint</button>
            </div>
            <div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 text-[10px] uppercase tracking-[.14em] text-[oklch(0.55_0.015_165)]">
              {DESTINATIONS.slice(0, 4).map((item) => <span key={item}>{item}</span>)}<span>+ more destinations</span>
            </div>
          </motion.div>
...
function HeroProductVisual() {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .9, delay: .25 }} className="relative mx-auto flex min-h-[470px] w-full max-w-[640px] items-center justify-center">
      <div className="absolute inset-x-[8%] top-[7%] h-[82%] rounded-[36px] border border-[oklch(0.74_0.17_162/0.13)] bg-[oklch(0.12_0.016_165/0.6)] shadow-[0_40px_120px_oklch(0.05_0.02_165/0.55)] backdrop-blur-xl" />
      <div className="absolute inset-x-[12%] top-[13%] h-[70%] rounded-[30px] border border-[oklch(0.74_0.17_162/0.12)] bg-[linear-gradient(145deg,oklch(0.16_0.02_165/.86),oklch(0.10_0.013_165/.78))]" />

      <div className="relative z-10 h-[340px] w-[340px] sm:h-[390px] sm:w-[390px]">
        <motion.div className="absolute inset-[17%] rounded-full border border-[oklch(0.74_0.17_162/0.25)]" animate={{ rotate: 360 }} transition={{ duration: 32, repeat: Infinity, ease: "linear" }} />
        <motion.div className="absolute inset-[25%] rounded-full border border-dashed border-[oklch(0.74_0.17_162/0.16)]" animate={{ rotate: -360 }} transition={{ duration: 24, repeat: Infinity, ease: "linear" }} />
        <motion.div className="absolute inset-[32%] rounded-full" animate={{ scale: [1, 1.06, 1], opacity: [.65, 1, .65] }} transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut" }} style={{ background: "radial-gradient(circle at 35% 30%, oklch(0.86 0.2 158 / .95), oklch(0.74 0.17 162 / .55) 45%, transparent 74%)", boxShadow: "0 0 80px oklch(0.74 0.17 162 / .35)" }} />

        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative w-[62%] rounded-[28px] border border-[oklch(0.98_0.005_160/.14)] bg-[oklch(0.08_0.012_165/.72)] p-5 shadow-2xl backdrop-blur-2xl">
