...
const INTAKES = ["January 2026", "April 2026", "September 2026", "January 2027", "April 2027", "September 2027"];
const PHASES = [
...
export default function OnboardingWizard({ onComplete }: OnboardingWizardProps) {
  ...
  const [form, setForm] = useState({
    ...
    documentsTotal: profile.documentsTotal, documentsVerified: profile.documentsVerified, homeLanguage: profile.homeLanguage ?? "",
  });
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (data.profile) setProfile({ ...nextProfile, ...data.profile });
      onComplete();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-[var(--shield-ink)]/95 backdrop-blur-md">
      ...
      <div className="relative mx-4 w-full max-w-lg">
        <div className="mb-8 flex items-center justify-center gap-2">{STEPS.map((s) => <div key={s.id} className={`rounded-full transition-all duration-300 ${s.id === step ? "h-2 w-8 bg-[oklch(0.74_0.17_162)]" : s.id < step ? "h-2 w-2 bg-[oklch(0.74_0.17_162/0.6)]" : "h-2 w-2 bg-[oklch(0.74_0.17_162/0.2)]"}`} />)}</div>
        <AnimatePresence mode="wait"><motion.div key={step} initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.2 }} className="rounded-2xl border border-[oklch(0.74_0.17_162/0.25)] bg-[oklch(0.18_0.022_165/0.95)] p-8">
          <div className="mb-6 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[oklch(0.74_0.17_162/0.15)] text-[oklch(0.74_0.17_162)]">{STEPS[step - 1].icon}</div><div><div className="text-[11px] uppercase tracking-wider text-[var(--shield-text-faint)]">Step {step} of {STEPS.length}</div><h2 className="text-xl font-semibold">{STEPS[step - 1].title}</h2></div></div>
          <div className="space-y-4">
            {step === 1 && <><Field label="Full name *" value={form.name} onChange={(v) => update("name", v)} placeholder="Your name" /><Field label="Email" value={form.email} onChange={(v) => update("email", v)} placeholder="your@email.com" type="email" /><Field label="Home language" value={form.homeLanguage} onChange={(v) => update("homeLanguage", v)} placeholder="e.g. Hindi, Tamil, Telugu" /></>}
          ...
          {saveError && <div className="mt-4 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-xs text-red-300">{saveError}</div>}
          <div className="mt-8 flex items-center justify-between"><button onClick={() => setStep((s) => Math.max(1, s - 1))} disabled={step === 1 || saving} className="flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm text-[var(--shield-text-dim)] disabled:opacity-30"><ChevronLeft className="h-4 w-4" />Back</button>{step < STEPS.length ? <button onClick={() => canAdvance() && setStep((s) => s + 1)} disabled={!canAdvance() || saving} className="flex items-center gap-2 rounded-xl bg-[oklch(0.74_0.17_162)] px-6 py-2.5 text-sm font-medium text-[oklch(0.14_0.018_165)]">Continue<ChevronRight className="h-4 w-4" /></button> : <button onClick={handleComplete} disabled={saving} className="flex items-center gap-2 rounded-xl bg-[oklch(0.74_0.17_162)] px-6 py-2.5 text-sm font-medium text-[oklch(0.14_0.018_165)]">{saving ? "Saving journey…" : <><Sparkles className="h-4 w-4" />Launch my agent</>}</button>}</div>
