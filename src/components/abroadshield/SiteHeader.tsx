...
export default function SiteHeader({ activeView, onViewChange, views, onTryAgent }: Props) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="fixed inset-x-4 top-20 z-50 rounded-3xl border border-[var(--shield-border)] bg-[oklch(0.11_0.015_165/0.98)] p-3 shadow-2xl backdrop-blur-2xl lg:hidden"><nav className="space-y-1">{publicNav.map((item) => <button key={item.id} onClick={() => handleNav(item.id)} className="block w-full rounded-xl px-3 py-3 text-left text-sm text-[var(--shield-text-dim)] hover:bg-white/5 hover:text-[var(--shield-text)]">{item.label}</button>)}<div className="grid grid-cols-2 gap-2 border-t border-[var(--shield-border)] pt-3"><button onClick={() => { setOpen(false); setAuthMode("login"); setAuthOpen(true); }} className="rounded-xl border border-[var(--shield-border)] px-4 py-3 text-sm font-medium">Sign in</button><button onClick={() => { setOpen(false); onTryAgent ? onTryAgent() : (setAuthMode("signup"), setAuthOpen(true)); }} className="rounded-xl bg-[oklch(0.98_0.005_160)] px-4 py-3 text-sm font-semibold text-[oklch(0.14_0.018_165)]">Start with the agent</button></div></nav></motion.div>}
      </AnimatePresence>
