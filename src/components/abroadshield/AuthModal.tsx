...
export default function AuthModal({ open, onClose, mode: initialMode = "signup" }: Props) {
  ...
  return <AnimatePresence>{open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-4">
    ...
    <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }} className="relative w-full max-w-md overflow-hidden rounded-3xl border border-[oklch(0.74_0.17_162/0.3)] as-glass-strong">
      ...
      <button type="button" onClick={onClose} className="absolute right-4 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--shield-border)] bg-[oklch(0.22_0.025_165/0.5)] text-[var(--shield-text-dim)] transition hover:text-[var(--shield-text)]"><X className="h-4 w-4" /></button>
      <div className="relative p-6 sm:p-8">
        ...
        <form className="space-y-3" onSubmit={handleCredentials}>
          {mode === "signup" && <div><label className="mb-1 block text-[11px] font-medium text-[var(--shield-text-dim)]" htmlFor="auth-name">Full name</label><div className="relative"><User className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--shield-text-dim)]" /><input id="auth-name" name="name" type="text" autoComplete="name" placeholder="Your name" className="w-full rounded-xl border border-[var(--shield-border)] bg-[oklch(0.18_0.022_165/0.6)] py-2.5 pl-10 pr-3 text-sm text-[var(--shield-text)] placeholder:text-[var(--shield-text-dim)] focus:border-[oklch(0.74_0.17_162/0.5)] focus:outline-none" /></div></div>}
          <div><label className="mb-1 block text-[11px] font-medium text-[var(--shield-text-dim)]" htmlFor="auth-email">Email</label><div className="relative"><Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--shield-text-dim)]" /><input id="auth-email" name="email" type="email" autoComplete="email" required placeholder="you@example.com" className="w-full rounded-xl border border-[var(--shield-border)] bg-[oklch(0.18_0.022_165/0.6)] py-2.5 pl-10 pr-3 text-sm text-[var(--shield-text)] placeholder:text-[var(--shield-text-dim)] focus:border-[oklch(0.74_0.17_162/0.5)] focus:outline-none" /></div></div>
          <div><label className="mb-1 block text-[11px] font-medium text-[var(--shield-text-dim)]" htmlFor="auth-password">Password</label><div className="relative"><Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--shield-text-dim)]" /><input id="auth-password" name="password" type="password" autoComplete={mode === "signup" ? "new-password" : "current-password"} required minLength={mode === "signup" ? 8 : undefined} placeholder="At least 8 characters" className="w-full rounded-xl border border-[var(--shield-border)] bg-[oklch(0.18_0.022_165/0.6)] py-2.5 pl-10 pr-3 text-sm text-[var(--shield-text)] placeholder:text-[var(--shield-text-dim)] focus:border-[oklch(0.74_0.17_162/0.5)] focus:outline-none" /></div></div>
          {error && <p role="alert" className="text-xs text-[oklch(0.66_0.19_22)]">{error}</p>}
        ...
        <p className="mt-4 text-center text-xs text-[var(--shield-text-dim)]">{mode === "signup" ? "Already have an account?" : "New to AbroadShield?"}{" "}<button type="button" onClick={() => { setMode(mode === "signup" ? "login" : "signup"); setError(""); }} className="font-semibold text-[oklch(0.85_0.19_158)] hover:underline">{mode === "signup" ? "Sign in" : "Create one"}</button></p>
