...
const TASKS: Record<PhaseId, Array<{ type: string; label: string; context: string; icon: typeof FileText }>> = {
  "pre-departure": [
    { type: "visa_check", label: "Review visa requirements", context: "Review the visa questions relevant to my destination and tell me what I should verify from official sources.", icon: ShieldCheck },
    { type: "document_check", label: "Check my documents", context: "Review my current study-abroad documents for gaps or inconsistencies and tell me what needs attention.", icon: FileText },
  ...
  arrival: [
    { type: "housing_search", label: "Plan housing", context: "Build my housing search criteria from my destination, budget and study situation. Do not invent live listings.", icon: Home },
    { type: "visa_check", label: "Check arrival requirements", context: "Explain the arrival and registration questions I should verify with the official authority.", icon: ShieldCheck },
    { type: "deadline_scan", label: "Scan arrival deadlines", context: "Review my known arrival dates and identify time-sensitive actions.", icon: CalendarClock },
  ],
  studying: [
  ...
  "job-success": [
    ...
    { type: "tailor_cv", label: "Tailor my CV", context: "Improve my CV using only facts already present in my profile. Never invent experience or metrics.", icon: FileText },
    { type: "deadline_scan", label: "Check career runway", context: "Review my known dates and identify job-search timing risks or missing information.", icon: CalendarClock },
  ],
};
...
export default function StageWorkspace({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  const tasks = TASKS[phase.id];
  const currentIndex = PHASES.findIndex((item) => item.id === currentPhase);
  const selectedIndex = PHASES.findIndex((item) => item.id === phase.id);
  ...
  async function run(task: typeof tasks[number]) {
    ...
    try {
      const res = await fetch("/api/abroadshield/tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ taskType: task.type, context: task.context, phase: phase.id, mode: phase.id === currentPhase ? "execute" : "plan" }) });
  ...
  const studentRole = phase.id === "pre-departure" ? "Keep your documents, dates, finances and application information accurate and complete." : phase.id === "arrival" ? "Complete prepared arrival actions, confirm local details and report changes to the agent." : phase.id === "studying" ? "Keep academic, financial and work-status information current so the plan can adapt." : "Provide accurate experience and preferences, review applications and approve outreach or submissions.";

  return <section className="mx-auto max-w-6xl px-5 py-7 sm:px-8">
    <div className="mb-5 rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-3xl"><div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--shield-text-faint)]">Journey</div><h1 className="mt-1 text-2xl font-semibold">Know where you are — and what comes next.</h1><p className="mt-2 text-sm leading-6 text-[var(--shield-text-dim)]">Your current stage controls execution. Every other stage stays visible so you can understand and prepare for the road ahead.</p></div><button onClick={() => onNavigate("agent")} className="inline-flex items-center gap-2 rounded-xl bg-[oklch(0.74_0.17_162)] px-4 py-2.5 text-xs font-semibold text-[oklch(0.12_0.016_165)]"><Bot className="h-4 w-4" />Ask the agent</button></div></div>
    <div className="mb-5 grid grid-cols-4 overflow-hidden rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)]">{PHASES.map((item,index)=>{const selected=item.id===phase.id;const current=item.id===currentPhase;const StageIcon=ICONS[item.id];return <button key={item.id} onClick={()=>{setSelectedPhase(item.id);setResult(null);}} className={\`relative px-3 py-4 text-left transition \${selected?"bg-[oklch(0.74_0.17_162/0.09)":"hover:bg-white/[0.02]"}\`}><div className="flex items-center gap-2"><StageIcon className="h-4 w-4 text-[oklch(0.74_0.17_162)]"/><span className="truncate text-[10px] font-semibold sm:text-xs">{item.name}</span></div><div className="mt-2 text-[9px] uppercase text-[var(--shield-text-faint)]">{current?"Current":index>currentIndex?"Up next":"Earlier"}</div>{selected&&<span className="absolute inset-x-0 bottom-0 h-0.5 bg-[oklch(0.74_0.17_162)]"/>}</button>})}</div>
    <div className="grid gap-5 lg:grid-cols-[1.05fr_.95fr]">
      <div className="rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] p-5"><div className="flex items-start gap-4"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[oklch(0.74_0.17_162/0.1)]"><Icon className="h-5 w-5 text-[oklch(0.74_0.17_162)]"/></div><div><div className="text-[9px] font-semibold uppercase tracking-[0.15em] text-[var(--shield-text-faint)]">Stage {selectedIndex+1} · {selectedIndex===currentIndex?"Current":selectedIndex>currentIndex?"Upcoming":"Earlier"}</div><h2 className="mt-1 text-xl font-semibold">{policy.title}</h2><p className="mt-2 text-sm leading-6 text-[oklch(0.85_0.19_158)]">{policy.mission}</p></div></div><div className="mt-5 rounded-xl border border-[var(--shield-border)] p-4"><div className="text-[9px] font-semibold uppercase tracking-[0.15em] text-[var(--shield-text-faint)]">How this stage fits your journey</div><p className="mt-2 text-sm leading-6 text-[var(--shield-text-dim)]">{policy.objective}</p><p className="mt-3 text-xs leading-5 text-[var(--shield-text-dim)]">{policy.systemFocus}</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-[var(--shield-border)] p-4"><div className="flex items-center gap-2 text-[9px] font-semibold uppercase tracking-[0.15em] text-[var(--shield-text-faint)]"><Bot className="h-3.5 w-3.5"/>AbroadShield</div><div className="mt-3 space-y-2 text-[11px] text-[var(--shield-text-dim)]">{policy.capabilities.slice(0,5).map(item=><div key={item} className="flex gap-2"><CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[oklch(0.74_0.17_162)]"/>{item.replaceAll("_"," ")}</div>)}</div></div><div className="rounded-xl border border-[var(--shield-border)] p-4"><div className="flex items-center gap-2 text-[9px] font-semibold uppercase tracking-[0.15em] text-[var(--shield-text-faint)]"><UserRound className="h-3.5 w-3.5"/>You</div><p className="mt-3 text-[11px] leading-5 text-[var(--shield-text-dim)]">{studentRole}</p></div></div></div>
      <div className="rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] p-5"><div className="mb-4"><div className="text-[9px] font-semibold uppercase tracking-[0.15em] text-[var(--shield-text-faint)]">Plan & act</div><h3 className="mt-1 text-lg font-semibold">{selectedIndex===currentIndex?"Work on what matters now":"Prepare for this stage"}</h3><p className="mt-1 text-xs leading-5 text-[var(--shield-text-dim)]">{selectedIndex===currentIndex?"Actions run against your current stage.":"Planning does not change your current stage."}</p></div><div className="space-y-2">{tasks.map(task=>{const TIcon=task.icon;return <button key={task.label} disabled={!!busy} onClick={()=>void run(task)} className="flex w-full items-center gap-3 rounded-xl border border-[var(--shield-border)] bg-[oklch(0.14_0.018_165)] p-3 text-left hover:border-[oklch(0.74_0.17_162/0.45)] disabled:opacity-60"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[oklch(0.74_0.17_162/0.08)]"><TIcon className="h-4 w-4 text-[oklch(0.74_0.17_162)]"/></span><span className="flex-1"><span className="block text-xs font-semibold">{task.label}</span><span className="block text-[10px] text-[var(--shield-text-faint)]">{selectedIndex===currentIndex?"Execute now":"Create a planning result"}</span></span>{busy===task.label?<span className="text-[10px] text-[oklch(0.74_0.17_162)]">Working…</span>:<ArrowRight className="h-3.5 w-3.5 text-[var(--shield-text-faint)]"/>}</button>})}</div>{result&&<div className="mt-4 rounded-xl border border-[oklch(0.74_0.17_162/0.28)] bg-[oklch(0.74_0.17_162/0.045)] p-4"><div className="mb-2 text-xs font-semibold">Latest result</div><pre className="max-h-72 overflow-auto whitespace-pre-wrap text-[11px] leading-5 text-[var(--shield-text-dim)]">{result}</pre></div>}</div>
    </div>
  </section>;
}
