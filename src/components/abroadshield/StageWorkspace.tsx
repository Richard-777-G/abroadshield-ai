...
export default function StageWorkspace({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  const currentPhase = (profile.currentPhase || "pre-departure") as PhaseId;
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-7 sm:px-8">
      <div className="mb-5 rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--shield-text-faint)]">Journey</div>
            <h1 className="mt-1 text-2xl font-semibold">Know where you are — and what comes next.</h1>
            <p className="mt-2 text-sm leading-6 text-[var(--shield-text-dim)]">Your current stage controls execution. Every other stage stays visible so you can understand and prepare for the road ahead.</p>
          </div>
          <button type="button" onClick={() => onNavigate("agent")} className="inline-flex items-center gap-2 rounded-xl bg-[oklch(0.74_0.17_162)] px-4 py-2.5 text-xs font-semibold text-[oklch(0.12_0.016_165)]"><Bot className="h-4 w-4" />Ask the agent</button>
        </div>
      </div>

      <div className="mb-5 grid grid-cols-2 overflow-hidden rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] sm:grid-cols-4">
        {PHASES.map((item, index) => {
          const selected = item.id === selectedPhase;
          const current = item.id === currentPhase;
          const StageIcon = ICONS[item.id];
          return (
            <button type="button" key={item.id} onClick={() => { setSelectedPhase(item.id); setResult(null); }} className={["relative px-3 py-4 text-left transition", selected ? "bg-[oklch(0.74_0.17_162/0.09)]" : "hover:bg-white/[0.02]"].join(" ")}>
              <div className="flex items-center gap-2"><StageIcon className="h-4 w-4 text-[oklch(0.74_0.17_162)]" /><span className="truncate text-[10px] font-semibold sm:text-xs">{item.name}</span></div>
              <div className="mt-2 text-[9px] uppercase text-[var(--shield-text-faint)]">{current ? "Current" : index > currentIndex ? "Up next" : "Earlier"}</div>
              {selected ? <span className="absolute inset-x-0 bottom-0 h-0.5 bg-[oklch(0.74_0.17_162)]" /> : null}
            </button>
          );
        })}
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-2xl border border-[var(--shield-border)] bg-[var(--shield-ink-2)] p-5">
