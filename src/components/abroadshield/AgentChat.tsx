...
import { Send, Sparkles, Bot, User, RotateCcw, Loader2, Check, Pencil, X, Copy, CheckCheck } from "lucide-react";
...
interface Message { id: string; role: "user" | "assistant"; content: string; pending?: boolean; action?: DraftAction; }

function isDraftMessage(content: string): boolean {
  const lower = content.toLowerCase();
  return lower.includes("approve to send") || lower.includes("approve / edit / decline") || /^subject:/im.test(content) || /^dear\b/im.test(content) || /```/.test(content);
}
...
function getAgentError(data: unknown, status: number): string {
  ...
  if (status === 403 && /credit card|billing|credits/i.test(error)) return "The agent is connected, but the AI provider account is blocked by billing. No task was falsely marked complete.";
