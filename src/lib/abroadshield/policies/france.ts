...
const verifiedEvidence = (input: Omit<StatutoryEvidence, "verificationState">): StatutoryEvidence => ({
  ...input,
  verificationState: "VERIFIED",
});

export const FRANCE_CVEC_RULE: PolicyRule<{ academicYear: string }, { feeEuros: number; portalUrl: string; evidence: StatutoryEvidence }> = {
  ...
  calculate: ({ academicYear }) => ({
    feeEuros: academicYear === "2026-2027" ? 105 : 0,
    portalUrl: FRANCE_OFFICIAL_SOURCES.cvec,
    evidence: FRANCE_CVEC_RULE.evidence,
  }),
};

export type VlsTsValidationStatus = "ON_TRACK" | "WARNING_30_DAYS" | "CRITICAL_14_DAYS" | "DUE_TODAY" | "OVERDUE" | "REQUIRES_MANUAL_CHECK";

export interface VlsTsValidationInput {
  entryDateIntoFrance: string;
  currentDate: string;
}
...
function toUtcMs(date: { year: number; month: number; day: number }): number {
...
export const calculateVlsTsValidationDeadline = (input: VlsTsValidationInput): VlsTsValidationResult => {
  const entry = parseIsoDate(input.entryDateIntoFrance);
  const current = parseIsoDate(input.currentDate);
  if (!entry || !current) {
    return {
      deadline: null,
      monthsWindow: 3,
      daysRemaining: null,
      taxStampCostEuros: null,
      status: "REQUIRES_MANUAL_CHECK",
      reason: "A valid ISO calendar date is required for both entry date and current date.",
      evidence: FRANCE_VLS_TS_EVIDENCE,
    };
  }
...
export interface StudentWorkBudgetInput {
  annualLoggedHours: number;
  calendarYear: number;
}

export interface StudentWorkBudgetResult {
  annualMaxHours: number | null;
  remainingHours: number | null;
  percentageUsed: number | null;
  complianceState: FrenchWorkComplianceState;
  evidence: StatutoryEvidence;
  reason?: string;
}
...
export const calculateFrenchWorkBudget = (input: StudentWorkBudgetInput): StudentWorkBudgetResult => {
  if (!Number.isFinite(input.annualLoggedHours) || input.annualLoggedHours < 0 || !Number.isInteger(input.calendarYear) || input.calendarYear < 2000) {
    return { annualMaxHours: null, remainingHours: null, percentageUsed: null, complianceState: "REQUIRES_MANUAL_CHECK", evidence: FRANCE_WORK_EVIDENCE, reason: "Logged hours must be a non-negative finite number and calendarYear must be valid." };
  }
  ...
  return {
    annualMaxHours,
    remainingHours: Math.max(0, Number(remainingHours.toFixed(2))),
    percentageUsed: Math.min(100, Math.max(0, percentageUsed)),
    complianceState,
    evidence: FRANCE_WORK_EVIDENCE,
  };
};
...
export const FRANCE_POLICY_RULES = [
  FRANCE_CVEC_RULE,
  {
    id: "fr-vls-ts-validation-3-months",
    title: "VLS-TS validation within 3 months of arrival",
    evidence: FRANCE_VLS_TS_EVIDENCE,
    calculate: calculateVlsTsValidationDeadline,
  },
  {
    id: "fr-student-work-964-hours",
    title: "Student work limit of 964 hours per year",
    evidence: FRANCE_WORK_EVIDENCE,
    calculate: calculateFrenchWorkBudget,
  },
] as const;
