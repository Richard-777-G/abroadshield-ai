...
function opportunity(id: string, overrides: Partial<Opportunity> = {}): Opportunity {
  return {
    canonicalId: id, providerId: "test-provider", sourceType: "job_board", sourceUrl: `https://example.com/jobs/${id}`,
    title: "Business Analytics Intern", employer: "Example Labs", location: "Paris", contractType: "internship",
    requiredSkills: ["Business Analytics"], retrievedAt: "2026-09-08T00:00:00.000Z", freshness: "fresh",
    applicationCapability: "L1", eligibility: "eligible",
    provenance: { provider: "test-provider", sourceUrl: `https://example.com/jobs/${id}`, retrievedAt: "2026-09-08T00:00:00.000Z" },
    ...overrides,
  };
}
...
describe("opportunity search engine", () => {
  ...
  test("isolates a failing adapter and surfaces its failure without leaking provider errors", async () => {
    ...
    expect(result.sourceErrors).toEqual([{ sourceId: "bad", error: "secret upstream detail" }]);
  });
  ...
  test("rejects invalid records at the engine boundary", async () => {
    ...
    const result = await searchOpportunities(request, [source]);
  ...
  test("does not treat unknown work authorization as eligible", async () => {
    ...
    const result = await searchOpportunities(request, [source]);
    expect(result.opportunities[0]?.eligibility).toBe("requires_work_authorization_check");
    expect(result.matches[0]?.constraints.find((item) => item.kind === "work_authorization")?.status).toBe("unknown");
  });
