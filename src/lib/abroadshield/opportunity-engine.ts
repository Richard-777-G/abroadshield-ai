...
export async function searchOpportunities(
  ...
  settled.forEach((settledResult, index) => {
    ...
    if (result.health === "failed") sourceErrors.push({ sourceId: adapter.sourceId, error: result.error ?? "Opportunity source failed." });
  ...
  const eligible = unique.map((opportunity) => {
    const result = evaluateOpportunityEligibility({ student: request.student, opportunity, asOf: request.asOf });
    return { opportunity: { ...opportunity, eligibility: result.eligibility }, result };
  });
  const matches = eligible.map(({ opportunity }) => matchOpportunity(opportunity, request.student));
  matches.sort((a, b) => fitRank(b.fit) - fitRank(a.fit));
  const byId = new Map(eligible.map(({ opportunity }) => [opportunity.canonicalId, opportunity]));
...
function dedupeKey(opportunity: Opportunity): string {
  return [normalizedKey(opportunity.employer), normalizedKey(opportunity.title), normalizedKey(opportunity.location), opportunity.contractType, normalizedKey(opportunity.applicationUrl ?? opportunity.sourceUrl)].join("|");
}
...
function deduplicateOpportunities(items: Opportunity[]): Opportunity[] {
  ...
  for (const item of items) {
    ...
    if (!existing || new Date(item.retrievedAt).getTime() > new Date(existing.retrievedAt).getTime()) seen.set(key, item);
  }
  return [...seen.values()];
}
