...
export const STAGE_POLICIES: Record<PhaseId, StagePolicy> = {
  "pre-departure": {
    ...
    capabilities: ["visa_check", "document_check", "deadline_scan", "draft_email", "cvec_payment"],
    systemFocus: "Prioritize official destination requirements, document evidence, appointment and application readiness, funding evidence, and time-critical pre-departure actions. France-specific CVEC workflows are enabled when the destination is France.",
  },
  arrival: {
    ...
    capabilities: ["housing_search", "document_check", "deadline_scan", "draft_email", "visa_check", "vlsts_validation", "caf_housing_check", "ameli_registration"],
    systemFocus: "Prioritize arrival formalities, accommodation, local setup and destination-specific post-arrival obligations. France VLS-TS, CAF and health-registration capabilities are enabled only for France.",
  },
  studying: {
    ...
    capabilities: ["deadline_scan", "document_check", "draft_email", "job_search", "housing_search", "visa_check", "work_rule_check"],
    systemFocus: "Prioritize academic deadlines, work restrictions, funding, housing continuity and current immigration conditions relevant to the student's status. France uses a deterministic work-budget capability for the statutory annual limit.",
  },
  "job-success": {
