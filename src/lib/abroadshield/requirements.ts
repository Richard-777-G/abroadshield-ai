import { COUNTRIES, type PhaseId, type CountryRule } from "@/components/abroadshield/data";
...
export function buildRequirementSnapshot(profile: Profile = {}): RequirementSnapshot {
  ...
  if (!country) {
    return { country: null, phase, requirements: [], verifiedDocuments, totalDocuments, readiness, summary: { critical: 0, high: 0, review: 0, ready: 0 } };
  }

  const requirements = country.checklist
    .filter((item) => item.phase === phase)
    .map((item, index): JourneyRequirement => {
  ...
  if (requirements.length === 0) {
    requirements.push({
      id: `${country.country}-${phase}-rules`, title: `${phase} rule review`, phase, status: "needs_review", priority: "info",
      reason: "No checklist items are currently configured for this stage in the destination rule table.",
      nextAction: "Ask the agent to review this stage against the official destination authority before taking action.", source: sourceFor(country, 0),
    });
  }
  ...
  const summary = {
    ...
    review: requirements.filter((r) => r.status === "needs_review").length,
