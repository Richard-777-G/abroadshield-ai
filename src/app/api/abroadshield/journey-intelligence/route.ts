...
export async function POST(request: Request) {
  try {
    ...
    const prompt = ["You are AbroadShield AI's Journey Intelligence layer.","Build an explainable, student-specific four-stage strategy from the authenticated profile and persisted journey history.","Do not invent facts. Surface missing information explicitly.","Current stage gets immediate priorities; future stages get preparation guidance.","For every stage provide objective, whyItMatters, abroadShieldWill, studentWill, prerequisites, risks, firstActions.","Also provide studentSummary, careerDirection, biggestUnknowns, next90Days.","Use persisted events and tasks to avoid recommending work already completed unless it needs follow-up.","Return valid JSON only.","PROFILE:",buildAgentContext(profile),"PERSISTED RECENT EVENTS:",recentEvents.map((e) => `- [${e.phase}] ${e.type}: ${e.title}${e.detail ? ` — ${e.detail}` : ""}`).join("\n") || "none yet","PERSISTED RECENT TASKS:",recentTasks.map((t) => `- [${t.phase}] ${t.status}: ${t.title}${t.completedAt ? ` — completed ${t.completedAt.toISOString()}` : ""}`).join("\n") || "none yet","CANONICAL STAGE POLICIES:",JSON.stringify(STAGE_POLICIES)].join("\n\n");
    ...
    console.error("journey-intelligence", error);
