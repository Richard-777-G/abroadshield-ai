...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json().catch(() => ({}))) as { taskType?: unknown; message?: unknown; context?: unknown; phase?: unknown; mode?: unknown };
    ...
    if (!taskType) return NextResponse.json({ ok: false, error: "Could not determine the task type. Specify a supported capability or provide a more specific request." }, { status: 400 });
    ...
    const result = await executeAgentTask(user.id, profile, { taskType, context: request, phase, mode });
