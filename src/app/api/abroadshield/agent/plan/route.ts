...
function inferTask(message: string, phase: string): Plan {
  ...
  const fallback: Record<string, { type: TaskType; title: string; reason: string }> = { "pre-departure": { type: "deadline_scan", title: "Find my next pre-departure action", reason: "No explicit task type was detected, so start with the current stage's next known date/action." }, arrival: { type: "visa_check", title: "Check my arrival requirements", reason: "No explicit task type was detected, so verify arrival/registration requirements first." }, studying: { type: "job_search", title: "Review current study-stage opportunities", reason: "No explicit task type was detected, so start with the most useful study-stage career action." }, "job-success": { type: "job_search", title: "Review my job-search next step", reason: "No explicit task type was detected, so start with the current job-success workflow." } };
...
const taskInstruction = (taskType: TaskType, profileContext: string, request: string, phase: string, connectorContext = "") => {
  ...
  const instructions: Record<TaskType, string> = {
    ...
    job_search: `${common}\nCreate a research-ready search specification. If no live job connector is available, explicitly return status needs_live_search rather than inventing jobs. Return {"status":"needs_live_search|shortlist","query":string,"criteria":string[],"roles":[],"nextAction":string}.`,
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (plan.taskType === "deadline_scan" || /check my email|read my email|recent emails/i.test(message)) {
      ...
      const detail = error instanceof Error ? error.message : "Task execution failed"; await db.journeyTask.update({ where: { id: task.id }, data: { status: "failed", result: JSON.stringify({ error: detail }) } }); await db.journeyEvent.create({ data: { userId: user.id, phase, type: "task_failed", title: plan.title, detail, metadata: JSON.stringify({ taskId: task.id, taskType: plan.taskType }) } }); return NextResponse.json({ ok: false, error: "The planned task failed.", taskId: task.id, plan }, { status: 502 });
    }
