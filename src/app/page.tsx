...
export default function Home() {
  ...
  const content = (
    <AnimatePresence mode="wait">
      <motion.div key={activeView} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
        ...
        {activeView === "journey" && <><ViewHero viewId="journey" /><JourneyExplorer /><DeadlineTimeline /><MemoryVault /></>}
  ...
  return <div className="relative flex min-h-screen flex-col bg-transparent">
    ...
    {isWorkspace ? (
      <AppShell activeView={activeView as WorkspaceView} onNavigate={handleWorkspaceNavigate}>{content}</AppShell>
    ) : (
      <>
        <SiteHeader activeView={activeView} onViewChange={navigateTo} views={VIEWS} onTryAgent={() => {
          if (session) setShowOnboarding(!profile.onboarded);
          else setShowAuthForDashboard(true);
        }} />
